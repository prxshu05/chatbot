        ...
        if "admissions" in user_query.lower():  # Auto-trigger contact form for admissions
            ...
            with st.spinner("Thinking... 🤔"):
                try:
                    ...
                    if not response or any(phrase in str(response).lower().strip() for phrase in [
                        "not currently available", "i don't know", "not provided", "unable to find",
                        "no information available", "not sure", "i'm not sure", "cannot determine",
                        "not found", "no data available", "no relevant information", "does not exist",
                        "not in my database", "unable to answer", "i have no record of that",
                        "please contact support", "please check with the admin", "ask the university directly"
                    ]):
                        try:
                            admin_message = send_email_to_admin(None, None, f"User Query: {user_query}")
                            st.write("It has been sent to the admin for review.")
                            st.success(admin_message)
                        except Exception as e:
                            st.error(f"❌ Error sending email to admin: {str(e)}")
                ...
                with st.spinner("Fetching response... 🤔"):
                    try:
                        ...
                        if not response or any(phrase in str(response).lower().strip() for phrase in [
                            ...
                            try:
                                admin_message = send_email_to_admin(None, None, f"User Query: {st.session_state.pending_query}")
                                st.write("It has been sent to the admin for review.")
                                st.success(admin_message)
                            except Exception as e:
                                st.error(f"❌ Error sending email to admin: {str(e)}")
